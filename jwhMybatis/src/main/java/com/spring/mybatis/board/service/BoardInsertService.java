...
public class BoardInsertService {
	...
	public String service(Board board, List<MultipartFile> files) {
	...
	public String service2(Board board, List<MultipartFile> files) {
		BoardFileDTO fileDTO = new BoardFileDTO();
		dao.increaseReord(board);
	...
	public void updateRe_id() {
		Board board = dao.findByReid(999);
		if(board!=null)	dao.updateReid(board.getId());
	}
}